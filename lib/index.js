  ...
  app.eventBus.subscribe({
    ...
    "/model/(.*)/change": {
      execute: function (payload, context) {
        publish("changeModel", {
          _cid: payload.key === "_cid" ? payload.oldValue : payload.newValue,
          key: payload.key,
    ...
    "/initializePubnub": {
      execute: function (options) {
        ...
        config.uuid = typeof options === "string" ? options : options.uuid;
        pubnub =  PUBNUB.init(config);

        if (options.channel) {
          app.eventBus.publish("/joinChannel", options.channel);
        }
      }
    }
